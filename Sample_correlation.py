...
def _run_single_case(L, beta, N, n_workers=None):
    ...
    if outcomes.shape[0] == 0:
        np.save(f"temp/MPS_CritIsing_L{L}_beta{beta}_Zcorr_N{N}_0_r.npy", np.array([]))
        np.save(f"temp/MPS_CritIsing_L{L}_beta{beta}_Zexp_N{N}_0_r.npy", np.array([]))
        return
    ...
    with Pool(
        ...
        for z_corr, z_exp in pool.imap(_process_outcome, outcomes, chunksize=chunksize):
            z_corr_list.append(z_corr)
            z_exp_list.append(z_exp)

    z_corr_arr = np.array(z_corr_list)
    z_exp_arr = np.array(z_exp_list)
    np.save(f"temp/Fast_MPS_CritIsing_L{L}_beta{beta}_Zcorr_N{N}_0_r.npy", z_corr_arr)
    np.save(f"temp/Fast_MPS_CritIsing_L{L}_beta{beta}_Zexp_N{N}_0_r.npy", z_exp_arr)
